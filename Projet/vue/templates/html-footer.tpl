  <img class="img" id="imgSuperman" src="vue/images/png/superman.png" alt="superman">
  <div class="footer">
    <div class="bandeau">
    </div>
    <footer>
      <div class="row">
        <div class="col-md-3 col-md-offset-1">
          <h3>Infos pratiques</h3>
        </div>
        <div class="col-md-3">
          <h3>Mon Compte</h3>
        </div>
        <div class="col-md-2">
          <h3>Suivez-nous sur les réseaux</h3>
        </div>
      </div>
      <div class="row">
        <div class="col-md-3 col-md-offset-1">
          <p>BLA</p>
          <p>BLA</p>
          <a href="index.php?controle=menu&action=administration">Page Administrateur</a>
        </div>
        <div class="col-md-3">
          <p>BLA</p>
          <p>BLA</p>
        </div>
        <div class="col-md-2 social-networks">
		  <div class="col-md-4 col-xs-1"><a href="http://facebook.fr"><img class="img-responsive" src="vue/images/png/facebook-white.png" alt="facebook"></a></div>
		  <div class="col-md-4 col-xs-1"><a href="http://twitter.fr"><img class="img-responsive" src="vue/images/png/twitter-white.png" alt="twitter"></a></div>
        </div>
      </div>
    </footer>
  </div>
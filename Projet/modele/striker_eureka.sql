...
--
-- Client :  127.0.0.1
-- Généré le :  Jeu 27 Juin 2019 à 18:41
...
(1, 'test@gmail.com', 'test123', 'doe', 'john', '12 rue blabla', 75420, 'Paris', 1),
...
INSERT INTO `commande` (`id_commande`, `id_client`, `total`) VALUES
(1, 1, 1493);
...
ALTER TABLE `commande`
  ...
  ADD KEY `id_client_commande` (`id_client`);
...
ALTER TABLE `produits_commande`
  ...
  ADD KEY `id_commande_listeprod` (`id_commande`),
  ADD KEY `id_produit_listeprod` (`id_produit`);
...
--
ALTER TABLE `produits_commande`
  MODIFY `id_produit_commande` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
--
...
--
ALTER TABLE `panier`
  ADD CONSTRAINT `FK_idclient_panier` FOREIGN KEY (`id_client`) REFERENCES `client` (`id_client`),
  ADD CONSTRAINT `FK_idproduit_panier` FOREIGN KEY (`id_produit`) REFERENCES `produit` (`id_produit`);
